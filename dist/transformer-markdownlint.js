...
var MarkdownLintTransformer = /** @class */ (function (_super) {
    __extends(MarkdownLintTransformer, _super);
    function MarkdownLintTransformer() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    MarkdownLintTransformer.prototype.parse = function (body) {
        var lintResults = [];
        for (var _i = 0, _a = body.split(/(\r\n)|\n|\r/g); _i < _a.length; _i++) {
            ...
            var pathAndLineAndColumnEndIndex = line.indexOf(" ");
            if (pathAndLineAndColumnEndIndex < 0) {
                continue;
            }
            var pathAndLineAndColumn = line.slice(0, pathAndLineAndColumnEndIndex);
            var locations = pathAndLineAndColumn.split(":");
            var path = locations[0];
            var startLine = 1 <= locations.length && Number.isInteger(parseInt(locations[1])) ? parseInt(locations[1]) : undefined;
            var ruleEndIndex = line.indexOf(" ", pathAndLineAndColumnEndIndex + 1);
            if (ruleEndIndex < 0) {
                continue;
            }
            var rule = line.slice(pathAndLineAndColumnEndIndex + 1, ruleEndIndex);
            var message = line.slice(ruleEndIndex + 1).replace(/^(.+?)\s?(\[.+\])*?$/, "$1");
            lintResults.push({
                path: path,
                message: message,
                rule: rule,
    ...
    return MarkdownLintTransformer;
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    transformer = new MarkdownLintTransformer();
...
/******/ 		var __webpack_exports__ = __webpack_require__.O(undefined, [736], () => (__webpack_require__(1581)))
